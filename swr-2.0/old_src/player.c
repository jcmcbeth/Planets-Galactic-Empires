...
void do_score(CHAR_DATA * ch, char *argument)
{
    ...
    ch_printf( ch, "&WStrength: &G%i", get_curr_str(ch) );
    ch_printf( ch, "&W  Constitution: &G%i", get_curr_con(ch) );
    ch_printf( ch, "&W  Dexterity: &G%i", get_curr_dex(ch) );
    send_to_char( "\n\r", ch );
    ch_printf( ch, "&WIntelligence: &G%i", get_curr_int(ch) );
    ch_printf( ch, "&W  Wisdom: &G%i", get_curr_wis(ch) );
    ch_printf( ch, "&W  Charisma: &G%i", get_curr_cha(ch) );
    ...
    ch_printf(ch, "&WArmor: &G %d", GET_AC(ch) );
...
/*    if      ( GET_AC(ch) >=   80 ) send_to_char( "naked",            ch );
    ...
    ch_printf( ch,
	"   &WCredits: &G%d\n\r" , ch->gold );
    ch_printf( ch, "&WHR: &G%i &WDR: &G%i &WBank: &G%ld\n\r",
     GET_HITROLL(ch), GET_DAMROLL(ch), ch->pcdata->bank );
    ...
    if ( !IS_NPC(ch) && ch->pcdata )
    {
    	...
    	for ( sn = 0; sn < top_sn ; sn++ )
    	   if ( ch->pcdata->learned[sn] > 0 && ch->pcdata->learned[sn] < 100 )
               ch_printf( ch,  "%s%s  ",
		IS_SET( skill_table[sn]->flags, SF_NOTEACH ) ? "&R" : "&G", skill_table[sn]->name );
        send_to_char( "\n\r" , ch );
	...
	ch_printf(ch, "&WBamfin:  %s\n\r&WBamfout: %s&x\n\r", ch->pcdata->bamfin, ch->pcdata->bamfout);
    }
