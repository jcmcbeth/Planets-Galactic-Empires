...
char * const planet_flags[] = { "invadable", "capturable", "playereditable", "nobomb", "hidden",
								"crapy", "revolting", "reserved2", "reserved3", "reserved4",
...
// Saves planets to file given the head of the list.
void save_planets()
{
   PLANET_DATA * planet;
   
   // Go through all the planets and save them
   for ( planet = first_planet; planet; planet = planet->next )
      save_planet( planet );
}
...
void do_setplanet( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( " coords flags popsupport\n\r", ch );
	return;
    }
   ...
   if ( !strcmp( arg2, "governed_by" ) )
   {
	...
	if(!strcmp(arg2, "popsupport")) {
		char support[MAX_INPUT_LENGTH];
		argument=one_argument(argument, support);
		if(support[0] != '\0') {
			planet->pop_support=atoi(support);
			send_to_char("Popular support changed.\n\r", ch);
		} else {
			send_to_char("Try putting a support level here.\n\r", ch);
		}
		save_planet( planet );
		return;
	}

	if ( !strcmp( arg2, "coords" ) ) {
		char x[MAX_INPUT_LENGTH];
		char y[MAX_INPUT_LENGTH];
		char z[MAX_INPUT_LENGTH];
		char buf[MAX_INPUT_LENGTH];

		argument = one_argument( argument, x );
		argument = one_argument( argument, y );
		argument = one_argument( argument, z );
		sprintf(buf, "Coords changed to, %i, %i, %i\n\r", atoi(x), atoi(y), atoi(z));
		send_to_char( buf, ch );
		planet->x=atoi(x);
		planet->y=atoi(y);
		planet->z=atoi(z);
	...
	if ( !str_cmp( arg2, "flags" ) )
	{
		...
		if ( argument[0] == '\0' || ( flag = get_planetflag( argument ) ) == -1 )
		{
			buf[0] = '\0';
			for ( i = 0; i < 32; i++ )
			{
				sprintf( buf, "%s %s", buf,  planet_flags[i] );
				if ( ( i + 1 ) % 5 == 0 )
					strcat( buf, "\r\n" );
			}
...
void do_showplanet( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf( ch, "&WSystem: &G%s&W\n\r", planet->starsystem->name ) ;

    ch_printf( ch, "&WCoords: &G%i %i %i\n\r", planet->x, planet->y, planet->z ) ;
    ...
    ch_printf( ch, "&WPlanet Size: &G%d\n\r", 
                   planet->size );
    ch_printf( ch, "&WCivilized: &G%d\n\r", pc ) ;
    ch_printf( ch, "&WWilderness: &G%d\n\r", pw ) ;
    ch_printf( ch, "&WFarmland: &G%d\n\r", pf ) ;
    ...
    ch_printf( ch, "&WPopular Support: &G%.2f\n\r", 
                   planet->pop_support );
    ch_printf( ch, "&WCurrent Weekly Revenue: &G%ld\n\r", 
                   get_taxes( planet) );
	if(!IS_SET(planet->flags, PLANET_INVADABLE)) {
		send_to_char("Planet is not invadable.\n\r", ch);
	} //phel
	if(!IS_SET(planet->flags, PLANET_CAPTURABLE)) {
		send_to_char("Planet is not capturable.\n\r", ch);
	} //phel
	if(IS_SET(planet->flags, PLANET_HIDDEN) && IS_IMMORTAL(ch)) {
		send_to_char("Planet is hidden.\n\r", ch);
	} //phel
	if(!IS_SET(planet->flags, PLANET_PLAYEREDITABLE) && IS_IMMORTAL(ch)) {
		send_to_char("Planet is not player editable.\n\r", ch);
	} //phel

   if ( IS_SET( planet->flags, PLANET_NOBOMB ) )
   {
      ch_printf( ch, "Planet may not be bombed.\r\n" );
   }

	if ( IS_SET( planet->flags, PLANET_CRAPY ) )	
		ch_printf( ch, "Planet is poorly designed.\r\n" );	
...
void do_makeplanet( CHAR_DATA *ch, char *argument )
{
	  ...
	  if ( !destok )
		...
		SET_BIT( location->room_flags , ROOM_NOPEDIT );
	        planet->citysize++;
             ...
             {
		location->name = STRALLOC( "Employment center" );
		strcpy( buf , "This is a employement center. Here a person can pick up delivery jobs to\n\r" );
		strcat( buf , "earn some extra credits.  Packages line the walls here needing to be\n\r" );
		strcat( buf , "delivered to various planets. If you are in need of a job just say 'yes'\n\r" );
		strcat( buf , "and the clerk with hand you a package for delivery.  \n\r" );
		location->description = STRALLOC(buf);
		location->sector_type = SECT_INSIDE;
		SET_BIT( location->room_flags , ROOM_INDOORS );
		SET_BIT( location->room_flags , ROOM_NO_MOB );
		...
		planet->citysize++;
	     ...
	     LINK( location , pArea->first_room , pArea->last_room , next_in_area , prev_in_area );
             ...
             if(rnum == 26) {
                 ...
                 xit = make_exit( location, troom, 4 );
                 xit->keyword           = STRALLOC( "" );
                 xit->description               = STRALLOC( "" );
                 xit->key                       = -1;
                 xit->exit_info         = 0;
                 xit = make_exit( troom, location, 5 );
                 xit->keyword           = STRALLOC( "" );
                 xit->description       = STRALLOC( "" );
                 xit->key               = -1;
                 xit->exit_info         = 0;
	...
	if(ch->pcdata->clan==NULL) {
		send_to_char("Helps to get a clan first.\n\r", ch);
		return;
    }

    if(!is_leader(ch)) {
	    send_to_char("Leaders only.\n\r",ch);
	    return;
    }
    ...
    if(pCount > 10) {
	send_to_char("You have too many planets as is, try conquering some.\n\r", ch);
	return;
    }
...
//basic planet setup
	SET_BIT(planet->flags, PLANET_CAPTURABLE);
	SET_BIT(planet->flags, PLANET_INVADABLE);
	SET_BIT(planet->flags, PLANET_PLAYEREDITABLE);
...
void do_planets( CHAR_DATA *ch, char *argument )
{
    ...
    for ( planet = first_planet; planet; planet = planet->next )
    {
        if ( planet->starsystem || IS_SET(planet->flags, PLANET_HIDDEN))
           continue;
...
/**
 * A command that will capture a planet for a clan.
 *
 * @author      Joel McBeth
 * @author      Unknown
 ...
 * @param       argument    The arguments supplied with the command.
 */
void do_capture( CHAR_DATA *ch , char *argument )
{
   ...
   old_clan = planet->governed_by;

   if(!IS_SET(planet->flags, PLANET_CAPTURABLE)) {
      send_to_char("This planet can't be captured.", ch);
      return;
   } //phel

   if ( planet->pop_support > 0 )
   {
      send_to_char ( "The population is not in favor of a new government.\n\r" , ch );
      return;
   }
   ...
   {    
      ...
      /* Check player's clan is at war with the owners of the planet. */
      if (clan_is_atwar(clan, planet->governed_by) == FALSE)
      {
         ch_printf(ch, "You must be at war with the government of the planet.\r\n");
         return;
      }
   ...
   if (old_clan != NULL)
   {
      ...
      /* Now to check to see if the clan was captured. */
      for (cPlanet = first_planet; cPlanet != NULL; cPlanet = cPlanet->next)
      {
            if ((cPlanet->governed_by != NULL) && (cPlanet->governed_by == old_clan))
            {
               last_planet = FALSE;
               break;
            }
      ...
      /* Check if the clan has no planets left. */
         if (last_planet == TRUE)
         {
            ship_count = 0;

            /* If for some reason they have negative funds. */
            treasury = UMAX(0, old_clan->funds);
            
            for (ship = first_ship; ship != NULL; clan_ship = ship->next)
            {
               /**
                * If the ship is not a mob ship, and the clan is not NULL and equals the old clan
                * give the ship to the new clan.
                */
               if ((ship->type != MOB_SHIP) &&
                     ((ship_clan = get_clan(ship->owner)) != NULL) &&
                     (ship_clan == old_clan))
               {
                     /* Just to make sure it is accurate, not like it can hurt. */
                     ship_count++;

                     STRFREE(ship->owner);

                     /* This gives the ship to the new clan. */
                     ship->owner = STRALLOC(clan->name);
               }
            }

            clan->spacecraft += ship_count;
            clan->funds += treasury;

            /**
             * I doesn't even think you can have clan vehicles. But if you can then once this clan is gone
             * it might cause complications.
             */

            sprintf(buf, "The %s has been captured by %s and\r\nhas captured %ld credits and %d of ships.", old_clan->name, clan->name, treasury, ship_count);
            echo_to_all(AT_RED, buf, ECHOTAR_ALL);

            /* Make sure this is after you reference their name, because remove clan frees the memory. */
            remove_clan(old_clan);
         }
   }
...
long get_taxes( PLANET_DATA * planet )
{
   ROOM_INDEX_DATA * room;
   long gain = 0;
   int platform = 0, garage = 0, trade = 0, bank = 0, gym = 0, mail = 0, info = 0,
       clone = 0, shipyard = 0, storage = 0, restaurant = 0, home = 0, hotel = 0,
       pawn = 0, supply = 0, bar = 0, medbay = 0;


   // Each citizen is taxed
   gain += planet->population * 50;
   // Each patch of wilderness offers resources worth credits
   gain += planet->wilderness * 60;

   // Go through each room and tally what types there are.
   // This might be slow and inefficient, but I don't think it would be slow enough to
   // notice. If it is I might have to abandon this idea or just keep up
   // with the types in the planet structure.
   for ( room = planet->area->first_room; room; room = room->next_in_area )
   {
      ...
      if ( IS_SET( room->room_flags, ROOM_INFO ) )
         info++;
      if ( IS_SET( room->room_flags, ROOM_CLONE ) )
         clone++;
   ...
   gain -= ( platform - 1 )          * 100;   
   gain += UMIN( garage, 10 )        * 10;
   gain += ( UMIN( trade, 10 ) - 1 ) * 10;
   gain += ( UMIN( bank, 5 ) - 1 )   * 100;
   gain += UMIN( gym, 20 )           * 5;
   gain -= ( mail - 1 )              * 100;
   gain -= ( info - 1 )              * 100;
   gain -= clone                     * 500;
   gain -= ( shipyard - 1 )          * 200;
   gain -= storage                   * 500;
   gain += UMIN( restaurant, 5 )     * 10;
   gain += UMIN( home, 5 )           * 100;
   gain += UMIN( hotel, 5 )          * 10;
   gain += UMIN( pawn, 10 )          * 10;
   gain += UMIN( bar, 5 )            * 10;   
   gain -= medbay                    * 100;

   gain -= planet->barracks          * 100;
   gain -= planet->controls          * 50;

   gain *= UMAX( planet->pop_support, 0 ) / 100;
...
int max_population( PLANET_DATA * planet )
{
   ...
   max = UMIN( planet->citysize, planet->farmland * 8 );
   max *= UMAX( planet->pop_support, 0 ) / 100;
...
long get_taxes( PLANET_DATA *planet )
{
      ...
      gain = 1000*planet->population;
      gain += 8000*resource;
      gain += planet->pop_support*500;
      
      gain -= 5000 * planet->barracks;
      gain -= 10000 * planet->controls;
      
      bigships = planet->controls/5;
      gain -= 50000 * bigships;
...
void extract_planet( PLANET_DATA * planet )
{
}
...
void do_remplanet( CHAR_DATA * ch, char * args )
{
/*
   PLANET_DATA * planet;
   char buf[MAX_STRING_LENGTH];

   if ( ( planet = get_planet( args ) ) == NULL )
   {
      ch_printf( ch, "That planet doesn't exist.\r\n" );
      return;
   }

   sprintf( buf, "%s%s", PLANET_DIR, planet->filename );
   if ( unlink( buf ) )   
      bug( "do_remplanet: unable to delete the planet." );   

   sprintf( buf, "%s%s", AREA_DIR, planet->area->filename );
   if ( unlink( buf ) )
      bug( "do_remplanet: unable to delete the area." );
*/
   // TODO: Finish this
/*
   extract_planet( planet );
