...
void do_makeblaster( CHAR_DATA *ch, char *argument )
{
    ...
    paf->modifier           = URANGE( 0, 2+scope, level/30 );
    paf->bitvector          = 0;
    paf->next               = NULL;
    LINK( paf, obj->first_affect, obj->last_affect, next, prev );
    ++top_affect;
...
void do_makelightsaber( CHAR_DATA *ch, char *argument )
{
	...
	if( ch->pcdata->learned[gsn_lightsaber_crafting] <= 0 )
	{
		send_to_char( "Huh?\n\r", ch );
		return;
	}
                ...
                for ( room = ship->first_room ; room ; room = room->next_in_ship )
		{
		   for ( guard = room->first_person; guard ; guard = guard->next_in_room )
			...
			for( slot = 0; slot < 5; slot++ )
				if( ship->module[slot] == MODULE_LOCKDOWN && strcmp( ch->name, ship->owner ) )
				{
					send_to_char( "&RAs you configure the launch codes you here a soft click and you feel yourself being scanned.&w\n\r", ch );
					sprintf( buf , "&C(GNET) %s: ALERT! %s! ALERT!  %s is attempting to hijack me!" , ship->name, ship->owner, ch->name);
					echo_to_all( AT_GOSSIP , buf, 0 );
					return;
				}
...
void do_propeganda ( CHAR_DATA *ch , char *argument )
{
   ...
   if ( IS_NPC(ch) || !ch->pcdata || !ch->pcdata->clan || !ch->in_room->area || !ch->in_room->area->planet )
   {
    ...
    if ( get_curr_cha(ch) + victim->top_level > ch->pcdata->learned[gsn_propeganda]  )
    {
...
void  clear_roomtype( ROOM_INDEX_DATA * location )
{
      ...
      REMOVE_BIT( location->room_flags, ROOM_CLONE );
      REMOVE_BIT( location->room_flags, ROOM_GYM );
      REMOVE_BIT( location->room_flags, ROOM_TRAIN );
      REMOVE_BIT( location->room_flags, ROOM_STORAGE );
}
...
void do_landscape ( CHAR_DATA *ch , char *argument )
{
	...
	if(ch->in_room->area == NULL) {
		send_to_char("Can't landscape in a ship.\n\r", ch);
		return;
	}

	if (!IS_SET(ch->in_room->area->planet->flags, PLANET_PLAYEREDITABLE)) {
		send_to_char("This planet isn't editable.\n\r", ch);
		return;
	} //phel
   ...
   if ( argument[0] == '\0' )
   {
	...
	send_to_char( "gym          - this room spawns training droids\n\r", ch );
	send_to_char( "cloning      - clone center(100k to make)\n\r", ch );
	send_to_char( "medical      - medical center for healing\n\r", ch );
	send_to_char( "train        - used to train stats\n\r", ch );
        ch_printf( ch, "storage      - you can store items here\r\n", ch );
	return;
   }
   ...
   else if ( !str_cmp( arg, "gym" ) )
   {
      location->area->planet->citysize++;
      location->sector_type = SECT_INSIDE;
      SET_BIT( location->room_flags , ROOM_GYM );
   } //phel
   else if (!str_cmp(arg, "cloning" ) )
   {
      if(ch->gold<100000) {
	send_to_char("A cloning room costs 100,000 credits", ch);
	return;
      }
      location->area->planet->citysize++;
      location->sector_type = SECT_INSIDE;
      SET_BIT( location->room_flags , ROOM_CLONE );
      ch->gold-=100000;
   } //phel
   else if ( !str_cmp( arg, "medical" ) )
   {
      location->area->planet->citysize++;
      location->sector_type = SECT_INSIDE;
      SET_BIT( location->room_flags , ROOM_MEDBAY );
   }
   else if ( !str_cmp( arg, "train" ) )
   {
      location->area->planet->citysize++;
      location->sector_type = SECT_INSIDE;
      SET_BIT( location->room_flags , ROOM_TRAIN );
   }
   else if ( !str_cmp( arg, "storage" ) )
   { 
      location->area->planet->citysize++;
      ...
      SET_BIT( location->room_flags, ROOM_NO_MOB );
      SET_BIT( location->room_flags, ROOM_STORAGE );
   } 
   else
   {
    	do_landscape( ch, "" );
...
void do_construction ( CHAR_DATA *ch , char *argument )
{
    ...
    if(!IS_SET(ch->in_room->area->planet->flags, PLANET_PLAYEREDITABLE)) {
		send_to_char("This planet isn't editable by players.\n\r", ch);
		return;
    } //phel
...
void do_bridge ( CHAR_DATA *ch , char *argument )
{
    ...
    {
       ch_printf( ch, "You can only do that on planets.\r\n" );
       return;
    }
...
void do_survey ( CHAR_DATA *ch , char *argument )
{
    ...
    room = ch->in_room;
       ...
       if ( IS_SET( room->room_flags , ROOM_BAR ) )
       	   ch_printf( ch, "&Y   This is a bar.\n\r" );

      if ( IS_SET( room->room_flags , ROOM_TRAIN ) )
       	   ch_printf( ch, "&Y   This is a stat training room.\n\r" );

      if ( IS_SET( room->room_flags , ROOM_GYM ) )
       	   ch_printf( ch, "&Y   This is a droid combat room.\n\r" );

      if ( IS_SET( room->room_flags , ROOM_MEDBAY ) )
       	   ch_printf( ch, "&Y   This is a medical room.\n\r" );

      if ( IS_SET( room->room_flags, ROOM_STORAGE ) )
          ch_printf( ch, "&Y   This is a storage room.\r\n" );
...
void do_quickarmor(CHAR_DATA *ch, char *argument) {
	char arg[MAX_INPUT_LENGTH];
	char arg2[MAX_INPUT_LENGTH];
	char arg3[MAX_INPUT_LENGTH];
	char buf[MAX_STRING_LENGTH];
	...
	if(arg2[0] == '\0') {
		send_to_char("&RUsage: Quickarmor <wearloc> <layer> <name>\n\r&w", ch);
		return;   
	}
	...
	switch(atoi(arg2)) {
		...
		case 8:
			...
			break;
		default:
			send_to_char("Layer should be a number from 1 to 8.\n\r", ch);
			return;
			break; //not needed, but looks better, I think.
	}
	...
	send_to_char("&GYou wave your hands and some armor appears.&w\n\r", ch);
}
...
void do_makemodule(CHAR_DATA *ch, char *argument) {
	...
	checkmetal, checkbattery, checkcircuit, checksuper;
	int moduletype;
	OBJ_DATA *obj;
	OBJ_DATA *metal;
	OBJ_DATA *circuit;
	...
	switch(ch->substate)
	{
		default:
			if (arg[0] == '\0')
			{
				send_to_char("&RUsage: makemodule <type>\n\r", ch);
                                send_to_char("Type: speed, fueltank, tractorbeam, manuever, cloak or lockdown.&w\n\r", ch );
//				send_to_char( "Type: speed, manuever, cloak or lockdown.&w", ch );
				return;
			}

                        if( !str_cmp(arg, "lockdown") )
                        	moduletype = MODULE_LOCKDOWN;
                        else if( !str_cmp(arg, "speed") )
                        	moduletype = MODULE_SPEED;
			 else if( !str_cmp(arg, "cloak") )
                                moduletype = MODULE_CLOAK;
			else if( !str_cmp(arg, "fueltank") )
				moduletype = MODULE_FUELTANK;
			else if( !str_cmp(arg, "tractorbeam") )
				moduletype = MODULE_TRACTORBEAM;
			else if( !str_cmp(arg, "manuever") )
				moduletype = MODULE_MANUEVER;
			else
			{
				send_to_char( "&RYou cannot make that moudle.\n\r", ch );
				send_to_char( "Usage: makemodule <type>\n\r", ch );
//                                send_to_char("Type: speed, jammer, tractorbeam, manuever, or lockdown.&w\n\r", ch );
				send_to_char( "Type: speed, fueltank, manuever, cloak or lockdown.&w", ch );
				return;
			}
	 		checktool = FALSE;
	 		checkoven = FALSE;
			...
			if(!checkbattery)
			{
				send_to_char("&RYou need a power source.\n\r", ch);
				return;
			}

                        if(!checksuper)
			{
				send_to_char("&RYou need a superconductor.\n\r", ch);
				return;
			}
			...
			if (number_percent() < chance)
			{
				send_to_char("&GYou begin the long process of creating a ship module.\n\r", ch);
				act(AT_PLAIN, "$n takes $s toolkit and some electronics and begins to work.", ch,
				NULL, argument , TO_ROOM);
	...
	if((!checkoven) || (!checksuper) || (!checkbattery) || (!checkcircuit) || (!checktool) || (!checkmetal))
	{
	...
	case MODULE_LOCKDOWN:
		strcpy(buf, "an automatic lockdown module" );
		break;
	case MODULE_SPEED:
		strcpy(buf, "a speed module" );
		break;
	 case MODULE_CLOAK:
                strcpy(buf, "a cloak module" );
                break;
	case MODULE_FUELTANK:
		strcpy(buf, "a fueltank module" );
		break;
	case MODULE_TRACTORBEAM:
		strcpy(buf, "an tractorbeam module" );
		break;
	case MODULE_MANUEVER:
		strcpy(buf, "a manuever module" );
		break;
	}
	...
	strcat(buf, " was dropped here.");
	obj->description = STRALLOC(buf);
	obj->value[0] = moduletype;
	obj->cost = 50000;
	SET_BIT( obj->wear_flags, 1 << 14 );
	obj = obj_to_char( obj, ch );
	send_to_char( "&GYou finish your work and hold up your newly created module.&w\n\r", ch);
	act( AT_PLAIN, "$n finishes creating a module.", ch, NULL, argument , TO_ROOM );
...
void do_installmodule(CHAR_DATA *ch, char *argument)
{
	...
	if( (ship = ship_from_cockpit(ch->in_room)) == NULL )
		...
		{
			send_to_char("&RYou must be in the cockpit or engine room of a ship to do that!\n\r",ch);
			return;
		}

	module = get_eq_char( ch, WEAR_HOLD );

	if( !module || module->item_type != ITEM_MODULE )
	{
		send_to_char("&RYou must be holding the module you want to install.&w\n\r", ch );
		return;
	}

	for( slot = 0; slot <= 4; slot++ )
		if( ship->module[slot] == MODULE_NONE )
			break;

	if( (slot + 1) > ship->modules || slot > 4 )
	{
		send_to_char("&RAll your ship module slots are full.&w\n\r", ch );
		return;
	}

   if ( ch->gold < 500000 )
   {
      ch_printf( ch, "You don't have enough credits." );
      return;
   }
	...
	if( number_percent( ) > chance )
	{
		send_to_char( "&RYou fail to install the module.&w\n\r", ch );
		return;
	}

   ch->gold -= 500000;
   ch_printf( ch, "It cost 500000 credits to install the module." );

	switch( module->value[0] )
	{
	case MODULE_LOCKDOWN:
		ship->module[slot] = MODULE_LOCKDOWN;
		break;
	case MODULE_SPEED:
		if( ship->realspeed >= 205 )
		{
			send_to_char( "This ship is too fast for this module.\n\r", ch );
			return;
		}

		ship->realspeed += 50;
		ship->module[slot] = MODULE_SPEED;
		break;
	case MODULE_FUELTANK:
		ship->module[slot] = MODULE_FUELTANK;
		ship->maxenergy += ship->maxhull * .05; 
		break;
	  case MODULE_CLOAK:
                ship->module[slot] = MODULE_CLOAK;
                break;
	case MODULE_TRACTORBEAM:
		ship->module[slot] = MODULE_TRACTORBEAM;
		ship->tractorbeam += 1;
		break;
	case MODULE_MANUEVER:
		if( ship->manuever >= 205 )
		{
			send_to_char( "This ship is too manuverable for this module.\n\r", ch );
			return;
		}

		ship->manuever += 50;
		ship->module[slot] = MODULE_MANUEVER;
		break;
	default:
		send_to_char("&RThe module you are holding is broken.&w\n\r", ch );
		return;
	}

	unequip_char( ch, module );
	separate_obj( module );
	extract_obj( module );
	save_ship(ship);
	send_to_char("&GYou install the ship module.&w\n\r", ch );
	save_ship( ship );

	if( module->value[1] == FALSE )
	{
...
void do_removemodule(CHAR_DATA *ch, char *argument)
{
	...
	if ( argument[0] == '\0' )
	{
		send_to_char( "Usage: removemodule <module slot>\n\r", ch );
		return;
	}
	...
	if( slot > 5 || slot < 1 )
	{
		send_to_char( "&RInvalid argument\n\r", ch );
		send_to_char( "Ships have slots numbered 1 through 5.&w\n\r", ch );
		return;
	}
	...
	if( ship->module[slot] == MODULE_NONE )
	{
		send_to_char( "&RThere is no module in that slot.&w\n\r", ch );
		return;
	}
	...
	if( number_percent( ) > chance )
	{
		send_to_char( "&RYou fail to remove the module.&w\n\r", ch );
		return;
	}

	level = IS_NPC(ch) ? ch->top_level : (int) (ch->pcdata->learned[gsn_removemodule]);

	if( (pObjIndex = get_obj_index( OBJ_VNUM_STEEL ) ) )
	{
		module = create_object(pObjIndex, 1);
		module->item_type = ITEM_MODULE;
		SET_BIT( module->wear_flags, ITEM_TAKE );
		module->level = level;
		STRFREE(module->name);
		STRFREE(module->short_descr);
		STRFREE(module->description);
		module->cost = 50000;
	}
	else
	{
		bug( "do_removemodule: OBJ_VNUM_STEEL not found!", 0 );
		send_to_char( "&RThe module explodes as you try to remove it!&w\n\r", ch );
		ship->module[slot] = MODULE_NONE;
		return;
	}

	module->value[0] = ship->module[slot];
	...
	module->short_descr = STRALLOC(buf);
	strcat(buf, " was dropped here.");
	module->description = STRALLOC(buf);
	SET_BIT( module->wear_flags, 1 << 14 );
	module = obj_to_char( module, ch );
	send_to_char("&GYou remove the ship module.&w\n\r", ch );
	save_ship( ship );
	if( module->value[1] == FALSE )
	{
		module->value[1] = TRUE;
...
/**
* This is the function that will be mapped to the plantbug skill. It places a bug
* in the room.
* @param ch	The character calling this function.
* @param args	The arguments the character provided when calling this function.
*/
void do_plantbug(CHAR_DATA * ch, char * args)
{
	OBJ_DATA * obj;
	int chance = 0;

	/* Check the character's substate for a timer. */
	switch (ch->substate)
	{
		default:
			/* Go through all the objects in the character's inventory. */
			for (obj = ch->first_carrying; obj; obj = obj->next_content)
			{
				/* Check if the item is a bug. */
				if (obj->item_type == ITEM_BUG)
				{
					ch_printf(ch, "You peer around the room as you try to discreetly plant a bug.\r\n");
					act(AT_PLAIN, "$n starts to suspicously peer around the room as he tries to hide something.", ch, NULL, NULL, TO_ROOM);

					/* Adds a timer to the character. */
					add_timer (ch, TIMER_DO_FUN, 3, do_plantbug, 1);
					return;
				}
			}

			/* If it gets here then a bug item was not found in the character's inventory. */
			ch_printf(ch, "&RYou do not have a bug to plant.\r\n");
			return;

		/* I'm assuming that this is for SUB_NONE */
		case 1:
			if (!ch->dest_buf)
				return;
		...
		/** If a user enters anything it will cancel the timer and this will be called. */
		case SUB_TIMER_DO_ABORT:
			DISPOSE(ch->dest_buf);
			ch->substate = SUB_NONE;
			ch_printf(ch, "&RYou are interupted and fail to finish placing your bug.\n\r");
			return;
	}

	ch->substate = SUB_NONE;

	chance = IS_NPC(ch) ? ch->top_level : (int)(ch->pcdata->learned[gsn_plantbug]);
	if (number_percent() > chance)
	{
		ch_printf(ch, "&RYou failed to find a good place to plant the bug.\r\n");
		return;
	}
	...
	/** Allocate memory. */
	CREATE(aBug, BUG_DATA, 1);               

	/** Add the bug to the list of bugs. */
	LINK(aBug, first_bug, last_bug, next, prev);

	/* aBug->bug_item = obj; */
	/* TODO: Fix it so the obj can be assigned. */
	aBug->bug_item = NULL;
	aBug->in_room = ch->in_room;
	aBug->owner = ch;

	ch_printf(ch, "You finish planting the bug.\r\n");

	learn_from_success(ch, gsn_plantbug);
}

void do_newmakeblaster( CHAR_DATA *ch, char *argument )
{
//    char arg[MAX_INPUT_LENGTH];
//    char buf[MAX_STRING_LENGTH];
//    int level, chance;
//    bool checktool, checkdura, checkbatt, checkoven, checkcond, checkcirc, checkammo,
//         checksniperscope, checkmicrocond, checkmircocirc, checknozzle;
//    OBJ_DATA *obj;
//    OBJ_INDEX_DATA *pObjIndex;
//    int vnum, power, scope, ammo;
//    AFFECT_DATA *paf;
...
//    	        if ( arg[0] == '\0' )
//                {
//                  send_to_char( "&RUsage: Makeblaster <blaster type> <name>\n\rAvailable blaster types:  Pistol, Rifle, Sniper Rifle\n\r&w", ch);
//                  return;   
//                }
//
//                if (!str_cmp (arg1, "rifle"))
//                {
//                  checktool = FALSE;
//                  checkdura = FALSE;
//                  checkbatt = FALSE;
...
//                  if ( !checkoven )
//                  {
//                    send_to_char( "&RYou need a small furnace mold the shell.\n\r", ch);
//                    return;
//                  }
//                
//                  if ( !checkcirc )
...
//                  obj->weight = 2+level/10;
//                  STRFREE( obj->name );
//                  /** Error: I dunno wtf this is for but it causes a compile error.
//                  strcpy( buf , arg2 ); */
//                  strcat( buf , " rifle");
//                  obj->name = STRALLOC( buf );
//                  strcpy( buf, arg2 );
...
//                  obj = obj_to_char( obj, ch );
//                                                            
//                  send_to_char( "&GYou finish your work and examine your newly create blaster rifle.&w\n\r", ch);
//                  act( AT_PLAIN, "$n finishes making $s new blaster rifle.", ch,
//                       NULL, argument , TO_ROOM );
//    
//                  learn_from_success( ch, gsn_makeblaster );
//                  learn_from_success( ch, gsn_makeblaster );
...
//                }
//                else
//                  if (!str_cmp(arg1, "pistol"))
//                  {
//                  checktool = FALSE;
//                  checkdura = FALSE;
//                  checkbatt = FALSE;
//                  checkoven = FALSE;
//                  checkmicrocond = FALSE;
//                  checkmicrocirc = FALSE;
//
//                  for ( obj = ch->last_carrying; obj; obj = obj->prev_content )
//                  {
//                  if (obj->item_type == ITEM_TOOLKIT)
...
//                  if ( !checkbatt )
//                  {
//                    send_to_char( "&RYou need a power source for your blaster pistol.\n\r", ch);
//                    return;
//                  }
//                
//                  if ( !checkoven )
...
//                  if ( !checkmicrocirc )
//                  {
//                    send_to_char( "&RYou need a micro circuit board to control the firing mechanism.\n\r", ch);
//                    return;
//                  }
//                
//                  if ( !checkmicrocond )
//                  {
//                    send_to_char( "&RYou still need a micro superconductor.\n\r", ch);
//                    return;
//                  }
// 
//    	          chance = IS_NPC(ch) ? ch->top_level
...
//                  if ( number_percent( ) < chance )
//    		      {
//    		        send_to_char( "&GYou begin the long process of making a blaster pistol.\n\r", ch);
//    		        act( AT_PLAIN, "$n takes $s tools and a small oven and begins to work on something.", ch,
//	                NULL, argument , TO_ROOM );
//		            add_timer ( ch , TIMER_DO_FUN , 10 , do_makeblaster , 1 );
//    		        ch->dest_buf   = str_dup(arg);
...
//                      checkmicrocond = TRUE;
//                    }
//                    if (obj->item_type == ITEM_CIRCUIT && checkmicrocirc == FALSE)
//                    {
//                      separate_obj( obj );
//                      obj_from_char( obj );
//                      extract_obj( obj );
...
//                  }
//    
//                  if ( ( !checktool ) || ( !checkdura ) || ( !checkbatt ) || ( !checkoven )  || ( !checkmicrocond ) || ( !checkmicrocirc) )
//                  {
//                      send_to_char( "&RYou examine your newly created blaster pistol very carefully.\n\r", ch);
//                      send_to_char( "&RYou notice a loose screw and tighten it.\n\r", ch);
//                      send_to_char( "&RYou accidenly crack the frame and everything falls to pieces.\n\r", ch);
//                      learn_from_failure( ch, gsn_makeblaster );
//                      return;
//                  }
//
...
//                  obj = obj_to_char( obj, ch );
//                                                            
//                  send_to_char( "&GYou finish your work and examine your newly create blaster pistol.&w\n\r", ch);
//                  act( AT_PLAIN, "$n finishes making $s new blaster pistol.", ch,
//                       NULL, argument , TO_ROOM );
//    
//                  learn_from_success( ch, gsn_makeblaster );
//                  learn_from_success( ch, gsn_makeblaster );
...
//                  if (obj->item_type == ITEM_SUPERCONDUCTOR)
//                    checkcond = TRUE;
//                  if (obj->item_type == ITEM_SNIPERSCOPE)
//                    checksniperscope = TRUE;
//                  if (obj->item_type == ITEM_NOZZLE)
//                    checknozzle = TRUE;
//                  }
//                
//                  if ( !checktool )
...
//                  if ( !checkcirc )
//                  {
//                    send_to_char( "&RYou need a circuit board to control the firing mechanism.\n\r", ch);
//                    return;
//                  }
//                
//                  if ( !checkcond )
//                  {
//                    send_to_char( "&RYou still need a superconductor.\n\r", ch);
//                    return;
//                  }
// 
//                   if ( !checksniperscope )
//                  {
//                    send_to_char( "&RYou need a scope to mount on the sniper rifle.\n\r", ch);
//                    return;
//                  }
//
//                  if ( !checknozzle )
//                  {
//                    send_to_char( "&RYou need a long nozzle.\n\r", ch);
//                    return;
//                  }
//
//    	          chance = IS_NPC(ch) ? ch->top_level
...
//                  if ( number_percent( ) < chance )
//    		      {
//    		        send_to_char( "&GYou begin the long process of making a sniper rifle.\n\r", ch);
//    		        act( AT_PLAIN, "$n takes $s tools and a small oven and begins to work on something.", ch,
//	                NULL, argument , TO_ROOM );
//		            add_timer ( ch , TIMER_DO_FUN , 10 , do_makeblaster , 1 );
//    		        ch->dest_buf   = str_dup(arg);
...
//                      checkmicrocirc = TRUE;
//                    }
//                    if (obj->item_type == ITEM_SNIPERSCOPE && checksniperscope == FALSE)
//                    {
//                      separate_obj( obj );
//                      obj_from_char( obj );
//                      extract_obj( obj );
//                      checksniperscope = TRUE;
//                    }
//                    if (obj->item_type == ITEM_NOZZLE && checknozzle == FALSE)
//                    {
//                      separate_obj( obj );
//                      obj_from_char( obj );
//                      extract_obj( obj );
...
//                  if ( ( !checktool ) || ( !checkdura ) || ( !checkbatt ) || ( !checkoven )  || ( !checkmicrocond ) || ( !checkmicrocirc) )
//                  {
//                      send_to_char( "&RYou examine your newly created sniper rifle very carefully.\n\r", ch);
//                      send_to_char( "&RYou make a slight adjustment on the scope.\n\r", ch);
//                      send_to_char( "&RYou break the scope, and the rest of the rifle falls to pieces.\n\r", ch);
//                      learn_from_failure( ch, gsn_makeblaster );
//                      return;
//                  }
//
...
//                  obj = obj_to_char( obj, ch );
//                                                            
//                  send_to_char( "&GYou finish your work and examine your newly create sniper rifle.&w\n\r", ch);
//                  act( AT_PLAIN, "$n finishes making $s new sniper rifle.", ch,
//                       NULL, argument , TO_ROOM );
//    
//                  learn_from_success( ch, gsn_makeblaster );
//                  learn_from_success( ch, gsn_makeblaster );
