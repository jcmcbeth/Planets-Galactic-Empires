...
// The first and last bugs of a doubly linked list.
BUG_DATA * first_bug;
BUG_DATA * last_bug;
...
void boot_db( bool fCopyOver )
{
    ...
    first_object	= NULL;
    last_object		= NULL;
    // Initilize the first bug in the list to NULL.
    first_bug = NULL;
    // Initilize the last bug in the list to NULL.
    last_bug = NULL;
    first_char		= NULL;
    last_char		= NULL;
    first_area		= NULL;
	...
	log_string("Assigning gsn's");
        ...
        ASSIGN_GSN( gsn_makemedpack  , "makemedpack" );
...
void log_string_plus( const char *str, sh_int log_type )
{
    ...
    {
	default:
	  to_channel( str + offset, CHANNEL_LOG, "Log", 200 );
	  break;
	case LOG_BUILD:
	  to_channel( str + offset, CHANNEL_BUILD, "Build", 200 );
	  break;
	case LOG_COMM:
	  to_channel( str + offset, CHANNEL_COMM, "Comm", 200 );
	  break;
	case LOG_ALL:
	  break;
    }
...
void fread_sysdata( SYSTEM_DATA *sys, FILE *fp )
{
	...
	case 'M':
	    KEY( "Multiplayers",   sys->multiplayers,      fread_string( fp ) );
	    break;
