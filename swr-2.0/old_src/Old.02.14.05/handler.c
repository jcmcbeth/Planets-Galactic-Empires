...
void extract_char( CHAR_DATA *ch, bool fPull )
{
    ...
    BUG_DATA * bugToExtract;
    // Go through all the bugs.
    for ( bugToExtract = first_bug; bugToExtract; bugToExtract = bugToExtract->next )
    {
       // If the character is the owner of the bug remove the bug from the list.
       if ( bugToExtract->owner == ch )
       {
          // Remove it from the list.
          UNLINK( bugToExtract, first_bug, last_bug, next, prev );
          // Free the memory.
          DISPOSE( bugToExtract );
       }
    }
