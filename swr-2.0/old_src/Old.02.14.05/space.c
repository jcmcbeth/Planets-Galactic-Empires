...
// Checks if a character is the owner of a ship or the leader of a clan
// that owns the ship.
...
void do_trajectory( CHAR_DATA *ch, char *argument )
{
	...
	if(arg3[0]=='\0')
		...
		if ((planet = get_planet(arg2)) &&
                    (planet->starsystem != NULL) &&
                    (ship->starsystem != NULL)
                    && (ship->starsystem == planet->system))
                {                    
			vx=planet->x;
			vy=planet->y;
			vz=planet->z;
...
{	    
   ...
   for ( mobship = ship->starsystem->first_ship; mobship; mobship = mobship->next_in_starsystem )
      if ( !str_cmp( planet->governed_by->name, mobship->owner ) && mobship->target == NULL )
      {  
         mobship->target = ship;
...
void do_unloadship( CHAR_DATA * ch, char * args )
{
   ...
   // but i'm not sure.
   if ( from->lastdoc != from->location || from->shipstate != SHIP_DOCKED || from->in_room )
   {
