...
int main( int argc, char **argv )
{
  ...
  /*  set_boot_time->hour   = 6;
    ...
    if (!fCopyOver) /* We have the port if copyover'd */
    {
...
//	    ||     d->idle > 28800 && IS_IMMORTAL(d->character)   /* 2 hrs  */
               )
	    {
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
	...
	if ( !d->character )
	{
	    ...
	    log_string( log_buf );
	    write_to_buffer( d, "Your playerfile is corrupt...Please notify galacticempires@hotmail.com.\n\r", 0 );
	    close_socket( d, FALSE );
	    return;
	}
	ch   = d->character;
	...
	write_to_buffer( d, "Thief                Pirate              Engineer\n\r", 0 );
	...
	if ( argument[0] == '\0' )
	{
        ...
        else if ( !str_prefix( argument , "medic" ) )
        {
            ...
            ch->pcdata->learned[gsn_makemedpack] = 50;
            ch->pcdata->num_skills = 5;
        ...
        else if ( !str_prefix( argument , "spy" ) )
        {
            ...
            ch->pcdata->learned[gsn_disguise] = 50;
...
/*
        else if ( !str_prefix( argument , "knight" ) )
        {
            ...
            ch->pcdata->learned[gsn_disarm] = 50;
            ch->pcdata->num_skills = 5;
	    sprintf( buf, "%s the intergalactic knight",ch->name );
	    set_title( ch, buf );
        }
*/
        else if ( !str_prefix( argument , "engineer" ) )
        {
            ch->pcdata->learned[gsn_spacecraft] = 50;
            ch->pcdata->learned[gsn_installmodule] = 50;
            ch->pcdata->learned[gsn_removemodule] = 50;
            ch->pcdata->learned[gsn_makemodule] = 50;
            ch->pcdata->learned[gsn_shipmaintenance] = 50;
            ch->pcdata->num_skills = 5;
	    sprintf( buf, "%s the engineer",ch->name );
	    set_title( ch, buf );
        }
/*
...
*/
	...
	write_to_buffer( d, "\n\rWould you like ANSI color support (Y/N)? ", 0 );
...
bool check_multi( DESCRIPTOR_DATA *d , char *name )
{
	...
	&&   str_cmp( name, dold->original
        ...
        && (!nifty_is_name(d->character->name, sysdata.multiplayers)) 
        && (!nifty_is_name(dold->character->name, sysdata.multiplayers)) 
	&& (!IS_IMMORTAL(dold->character)) )
	{
	        const char *ok = "0.0.0.0";
	        const char *ok2 = "0.0.0.0";
	        int iloop;
...
/*  make_color_sequence(NULL, NULL, NULL);*/
  for ( ; *prompt; prompt++ )
  {
	...
	*pbuf = '\0';
	...
	break;
     case 'x': //courtesy of phel
	stat = ch->hit;
	break;
     case 'X': //courtesy of phel
	stat = ch->max_hit;
	break;
     case 'y': //courtesy of phel
	stat = ch->move;
	break;
     case 'Y': //courtesy of phel
	stat = ch->max_move;
