...
//	    affect_loc_name( paf->location ), affect_bit_name( obj->affected_by ) );
...
ch_printf( ch,
          "Top Level: %d     Align: %d  AC: %d  Gold: %d  Bank: %ld\n\r",
	victim->top_level,   victim->alignment,
        GET_AC(victim),      victim->gold, (IS_NPC(victim) == FALSE) ? victim->pcdata->bank : 0 );
    ...
    if (IS_NPC(ch) == FALSE)
    {
        ch_printf(ch, "Supports: %s\r\n", (victim->pcdata->supports[0] != '\0') ? victim->pcdata->supports : "no one");
        ch_printf(ch, "Supporters: %s\r\n", get_supporters_string(victim));
    }

    ch_printf(ch, "Bestowments: %s\r\n", get_powers_string(ch));       
    ch_printf( ch, "Short description: %s\n\rLong  description: %s",
	victim->short_descr,
...
void do_bestow( CHAR_DATA *ch, char *argument )
{
    ...
    /**
     * Bestowments were being used entirely incorrect in clans so this
     * will be disabled until someone decides to reimplement it. Don't forget to
     * reimplement it in interpret!
     */
    return;
...
//setmulti and showmulti by phel
void do_setmulti( CHAR_DATA *ch, char *argument ) {
	STRFREE(sysdata.multiplayers);
	sysdata.multiplayers = STRALLOC(argument);
	send_to_char("Done.\n\r", ch);
	return;
}

void do_showmulti( CHAR_DATA *ch, char *argument ) {
	ch_printf(ch, "Allowed multiplayers: %s\n\r", sysdata.multiplayers);
}
