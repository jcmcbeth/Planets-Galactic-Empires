...
/**
 * Loads the objects stored in a player's house.
 *
 * @author  Joel McBeth
 * @version 1.0     02/12/05
 *
 * @param   ch      The character who has a house that should be loaded.
 */
void load_home(CHAR_DATA* ch)
{
    ...
    if (room == NULL)
    {
        bug("load_home: The player has a NULL home.", 0);
        return;
    }

    /* Get the filename of the file where the player's items are stored. */
    sprintf(filename, "%s%c/%s.home", PLAYER_DIR, tolower(ch->name[0]), capitalize(ch->name));

    home_file = fopen(filename, "r");

    if (home_file == NULL)
    {
        sprintf(buf, "Unable to open home file for %s at %s.", ch->name, filename);
        bug(buf);
        return;
    }

    /**
     * This is it reset the rgObjNest. For some reason to correctly
     * nest objects they need to be stored globably first.
     */
    ...
    for (;;)
    {
        letter = fread_letter(home_file);

        /* I don't know why this is done. */
        if (letter == '*')
        {
            fread_to_eol(home_file);
            continue;
        }

        /* Wrong format if this is not found. */
        if (letter != '#')
        {
            sprintf(buf, "load_home: # not found in %s", filename);
            bug(buf, 0);
            
            break;
        }

        word = fread_word(home_file);

        /* If object data is found read it. */
        if (str_cmp(word, "OBJECT") == FALSE)
        {
            obj = fread_obj_simple(home_file);

            obj_to_room(obj, room);
        }        
        else if (str_cmp(word, "END") == FALSE)        
        {
            /* If END is found then we are done reading the file. */
            break;
        }
        else
        {
            /* Wrong format if OBJECT or END is not found. */
            sprintf(buf, "load_home: file has wrong format: %s", filename);
            bug(buf, 0);
...
void fread_char( CHAR_DATA *ch, FILE *fp, bool preload )
{
	...
	case 'S':
		...
		if ( !preload )
		{
		  strcpy(site, fread_word(fp));
		  ch->site = STRALLOC(site);
...
/**
 * Reads an object from an open file.
 *
 * @author  Joel McBeth
 * @author  Unknown
 * @version 1.0     02/12/05
 ...
 */
OBJ_DATA* fread_obj_simple(FILE* fp)
{
    OBJ_DATA *obj;
    char *word;
    int iNest, pafmod;
    ...
    for ( ; ; )
    {
	    ...
	    case 'V':
		    ...
		    break;
                ...
                /* I'd think this would overwrite properties read from the file if they preceeded this. */
	        if ( !str_cmp( word, "Vnum" ) )
	        {
		    int vnum;
        ...
        /* If the property was not found then the file is in the wrong format. */
	if ( !fMatch )
	{
	    EXTRA_DESCR_DATA *ed;
	    AFFECT_DATA *paf;
    ...
    /* If it correctly reads the file it should never get here. */
    return NULL;
}
