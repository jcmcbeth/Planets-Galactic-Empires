...
void reset_all( )
{
   ...
   SHIP_DATA *ship;
	...
	for(system = first_starsystem; system; system = system->next)
        {
		if(!IS_SET(system->flags, SPACE_NOASTEROIDS))
                {
			make_asteroid(system);
			make_asteroid(system);
			make_asteroid(system);
			make_asteroid(system);
			make_asteroid(system);
	...
	{
		for( pRoomIndex = ship->first_room; pRoomIndex; pRoomIndex = pRoomIndex->next_in_ship )
                {
                	if( IS_SET( pRoomIndex->room_flags, ROOM_MEDBAY ) )
                        {
				found = FALSE;

				for( mob = pRoomIndex->first_person; mob ;mob = mob->next_in_room )
				{
					if( IS_NPC( mob ) && mob->pIndexData && mob->pIndexData->vnum == MOB_VNUM_DOCTOR )
					found = TRUE;
	 			}

                                if( found == FALSE )
                                {
					if( !(pMobIndex = get_mob_index(MOB_VNUM_DOCTOR)) )
                                        {
        	   				bug( "Reset_all: Missing mob MOB_VNUM_DOCTOR" );
                                                return;
					}
					...
					if( room_is_dark(pRoomIndex) )
						SET_BIT(mob->affected_by, AFF_INFRARED);
                                        ...
                                        if ( (pObjIndex = get_obj_index(OBJ_VNUM_TOOLKIT)) )
					{
                                        	obj = create_object(pObjIndex, 1);
                                                obj = obj_to_char(obj, mob);
                                                SET_BIT(obj->extra_flags, ITEM_INVENTORY);
                                        }
                                        if ( (pObjIndex = get_obj_index(OBJ_VNUM_PLASTIC)) )
					{
                                        	obj = create_object(pObjIndex, 1);
                                                obj = obj_to_char(obj, mob);
                                                SET_BIT(obj->extra_flags, ITEM_INVENTORY);
                                        }
                                        if ( (pObjIndex = get_obj_index(OBJ_VNUM_CHEMICALS)) )
					{
                                        	obj = create_object(pObjIndex, 1);
                                                obj = obj_to_char(obj, mob);
                                                SET_BIT(obj->extra_flags, ITEM_INVENTORY);
                                        }
                                        if ( (pObjIndex = get_obj_index(OBJ_VNUM_LATEX)) )
					{
                                        	obj = create_object(pObjIndex, 1);
                                                obj = obj_to_char(obj, mob);
                                                SET_BIT(obj->extra_flags, ITEM_INVENTORY);
       ...
       if ( d )
       {
         ...
         case 0:
            ...
            if( pRoomIndex )
            {
               ...
               if ( (pMobIndex = get_mob_index(MOB_VNUM_ALIEN)) && IS_SET(dPlanet->flags, PLANET_INVADABLE) )
               {
   ...
   {    
     ...
     {    
        ...
        if ( IS_SET(pRoomIndex->room_flags, ROOM_GYM ) )
             vnum = MOB_VNUM_DROID;
        if ( IS_SET(pRoomIndex->room_flags, ROOM_MEDBAY ) )
             vnum = MOB_VNUM_DOCTOR;
        if ( IS_SET(pRoomIndex->room_flags, ROOM_TRAIN ) )
             vnum = MOB_VNUM_TRAINER;
	    ...
	    for ( mob = pRoomIndex->first_person; mob ;mob = mob->next_in_room )
	    {
		   ...
		   //objs added to the shoppies by phel part one.
	           onum = number_range( 39, 40  );
                   if ( (pObjIndex = get_obj_index(onum)) )
                   {
                     obj = create_object(pObjIndex, 1);
                     obj = obj_to_char(obj, mob);
                   ...
                   //objs added to the shoppies by phel part one.5.
                   onum = number_range( 42, 48  );
                   if ( (pObjIndex = get_obj_index(onum)) )
                   {
                     obj = create_object(pObjIndex, 1);
                     obj = obj_to_char(obj, mob);
		   ...
		   //objs added to the shoppies by phel part two.
                   if ( (pObjIndex = get_obj_index(126)) )
                   {
                     obj = create_object(pObjIndex, 1);
                     obj = obj_to_char(obj, mob);
                     SET_BIT(obj->extra_flags, ITEM_INVENTORY);
                   }
		   //objs added to the shoppies by phel part three.
                   if ( (pObjIndex = get_obj_index(127)) )
                   {
                     obj = create_object(pObjIndex, 1);
                     obj = obj_to_char(obj, mob);
                     SET_BIT(obj->extra_flags, ITEM_INVENTORY);
	       ...
	       if ( vnum == MOB_VNUM_DROID ) {
                    if ( (pObjIndex = get_obj_index(109)) ) { // we like batons. phel
			obj = create_object(pObjIndex, 1);
                        obj = obj_to_char(obj, mob);
                        equip_char( mob, obj, WEAR_WIELD );
		    }
