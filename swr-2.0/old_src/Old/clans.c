...
/**
 * An immortal command that will edit the values of a clan.
 *
 * @author  Joel McBeth
 * @version 1.0     02/08/05 
 *
 ...
 */
void do_clset(CHAR_DATA* ch, char* args)
{
}

/**
 * An immortal command that will show the status of a clan.
 *
 * @author  Joel McBeth
 * @version 1.0     02/08/05 
 *
 ...
 * @param   args    The arguments supplied with the command. 
 */
void do_clstatus(CHAR_DATA* ch, char* args)
{
}
 ...
 * @param   args    The arguments supplied with the command.
 */
void do_found(CHAR_DATA* ch, char* args)
{    
    CHAR_DATA* temp_ch;
    CLAN_DATA* clan;
    ...
    for (temp_ch = first_char; temp_ch; temp_ch = temp_ch->next)
    {
        ...
        if ((temp_ch != ch) &&
            (IS_NPC(ch) == FALSE) &&
            (str_cmp(ch->name, temp_ch->supports) == FALSE))
        {            
            num_supporters++;
        }
    }    
 ...
 * @param   args    The arguments supplied with the command.
 */
void do_support(CHAR_DATA* ch, char* args)
{
    CHAR_DATA* victim;
    ...
    if (ch->desc == NULL)
    {
        ch_pritnf(ch, "You can't do that right now.\r\n");
        return;
    }
    ...
    {
        ch_printf(ch, "You may not support him.\r\n");
        return;
    }

    /* Make sure the character doesn't have a similar hostname to other supporters either. */
    for (ref_char = first_char; ref_char != NULL; ref_char = ref_char->next)
    {
        if ((IS_NPC(ref_char) == FALSE) &&
            (same_host(victim->desc, ch->desc->host) == TRUE))
        {
            ch_printf(ch, "You may not support him.\r\n");
            return;
        }
    ...
    /* If you support someone already inform them and yourself that you no longer do. */
    if (ch->pcdata->supports[0] != '\0')
    {
        ch_printf(ch, "You no longer support %s.\r\n", ch->pcdata->supports);
        
        old_victim = get_char_world(ch->pcdata->supports);
        if (old_victim != NULL)
        {
    ...
    act(AT_PLAIN, "$n shows $s support to $N.", ch, NULL, victim, TO_ROOM);
 ...
 * @param   args    The arguments supplied with the command.
 */
void do_clandescription(CHAR_DATA* ch, char* args)
{
    CLAN_DATA* clan;    
 ...
 *
 * @returns <code>TRUE</code> if the host should be consdiered the same, <code>FALSE</code> otherwise.
 */
